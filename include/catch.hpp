...
struct SourceLineInfo {
    ...
    SourceLineInfo(const char* _file, std::size_t _line) noexcept : file(_file), line(_line) {}

    SourceLineInfo(const SourceLineInfo& other) = default;
    SourceLineInfo& operator=(const SourceLineInfo&) = default;
    SourceLineInfo(SourceLineInfo&&) noexcept = default;
    SourceLineInfo& operator=(SourceLineInfo&&) noexcept = default;

    bool empty() const noexcept { return file[0] == '\0'; }
    bool operator==(const SourceLineInfo& other) const noexcept;
    bool operator<(const SourceLineInfo& other) const noexcept;
...
std::ostream& operator<<(std::ostream& os, const SourceLineInfo& info);
...
template<typename T>
const T& operator+(const T& value, StreamEndStop) {
    return value;
}
}  // namespace Catch
...
struct RegistrarForTagAliases {
    RegistrarForTagAliases(const char* alias, const char* tag, const SourceLineInfo& lineInfo);
};
...
struct ITestCaseRegistry {
    ...
    virtual const std::vector<TestCase>& getAllTestsSorted(const IConfig& config) const = 0;
};

bool isThrowSafe(const TestCase& testCase, const IConfig& config);
bool matchTest(const TestCase& testCase, const TestSpec& testSpec, const IConfig& config);
std::vector<TestCase> filterTests(
    const std::vector<TestCase>& testCases, const TestSpec& testSpec, const IConfig& config);
const std::vector<TestCase>& getAllTestCasesSorted(const IConfig& config);
...
class StringRef {
  ...
  private:
    static constexpr const char* const s_empty = "";

    const char* m_start = s_empty;
    size_type m_size = 0;
  ...
  public:  // construction
    ...
    constexpr StringRef(const char* rawChars, size_type size) noexcept :
  ...
  public:  // operators
    auto operator==(const StringRef& other) const noexcept -> bool;
    auto operator!=(const StringRef& other) const noexcept -> bool { return !(*this == other); }
...
auto operator+=(std::string& lhs, const StringRef& sr) -> std::string&;
auto operator<<(std::ostream& os, const StringRef& sr) -> std::ostream&;

constexpr auto operator"" _sr(const char* rawChars, std::size_t size) noexcept -> StringRef {
    return StringRef(rawChars, size);
}
}  // namespace Catch

constexpr auto operator"" _catch_sr(const char* rawChars, std::size_t size) noexcept
    -> Catch::StringRef {
...
#define INTERNAL_CATCH_NTTP_REGISTER0(TestFunc, signature)          \
    ...
    void reg_test(TypeList<Type>, Catch::NameAndTags nameAndTags) { \
        Catch::AutoReg(                                             \
            Catch::makeTestInvoker(&TestFunc<Type>),                \
...
#define INTERNAL_CATCH_NTTP_REGISTER(TestFunc, signature, ...)         \
    ...
    void reg_test(Nttp<__VA_ARGS__>, Catch::NameAndTags nameAndTags) { \
        Catch::AutoReg(                                                \
            Catch::makeTestInvoker(&TestFunc<__VA_ARGS__>),            \
            CATCH_INTERNAL_LINEINFO,                                   \
            Catch::StringRef(),                                        \
            nameAndTags);                                              \
    }
...
#define INTERNAL_CATCH_NTTP_REGISTER_METHOD0(TestName, signature, ...)                          \
    ...
    void reg_test(TypeList<Type>, Catch::StringRef className, Catch::NameAndTags nameAndTags) { \
        Catch::AutoReg(                                                                         \
            Catch::makeTestInvoker(&TestName<Type>::test),                                      \
            CATCH_INTERNAL_LINEINFO,                                                            \
            className,                                                                          \
            nameAndTags);                                                                       \
    }
...
struct NameAndTags {
    NameAndTags(
        const StringRef& name_ = StringRef(), const StringRef& tags_ = StringRef()) noexcept;
    StringRef name;
    StringRef tags;
};
...
struct AutoReg : NonCopyable {
    AutoReg(
        ITestInvoker* invoker, const SourceLineInfo& lineInfo, const StringRef& classOrMethod,
        const NameAndTags& nameAndTags) noexcept;
    ~AutoReg();
};
...
auto makeStream(const StringRef& filename) -> const IStream*;
...
class ReusableStringStream : NonCopyable {
  ...
  public:
    ...
    template<typename T>
    auto operator<<(const T& value) -> ReusableStringStream& {
        *m_oss << value;
        return *this;
    }
...
struct IMutableEnumValuesRegistry {
    ...
    virtual const Detail::EnumInfo& registerEnum(
        StringRef enumName, StringRef allEnums, const std::vector<int>& values) = 0;

    template<typename E>
    const Detail::EnumInfo& registerEnum(
...
typename std::enable_if<
    !std::is_enum<T>::value && !std::is_base_of<std::exception, T>::value, std::string>::type
convertUnstreamable(const T&) {
    return Detail::unprintableString;
}
template<typename T>
typename std::enable_if<
    !std::is_enum<T>::value && std::is_base_of<std::exception, T>::value, std::string>::type
convertUnstreamable(const T& ex) {
    return ex.what();
}
...
template<>
struct StringMaker<const wchar_t*> {
...
struct StringMaker<signed char[SZ]> {
    static std::string convert(const signed char* str) {
        return ::Catch::Detail::stringify(std::string{reinterpret_cast<const char*>(str)});
    }
};
template<int SZ>
struct StringMaker<unsigned char[SZ]> {
    static std::string convert(const unsigned char* str) {
        return ::Catch::Detail::stringify(std::string{reinterpret_cast<const char*>(str)});
    }
};
...
std::string rangeToString(const std::vector<bool, Allocator>& v) {
    ReusableStringStream rss;
    rss << "{ ";
    bool first = true;
    for (bool b : v) {
...
struct StringMaker<
    ...
    static std::string convert(const R& range) { return rangeToString(range); }
};

template<typename T, int SZ>
struct StringMaker<T[SZ]> {
    static std::string convert(const T (&arr)[SZ]) { return rangeToString(arr); }
};
...
struct StringMaker<std::chrono::duration<Value, Ratio>> {
    static std::string convert(const std::chrono::duration<Value, Ratio>& duration) {
        ReusableStringStream rss;
...
struct StringMaker<std::chrono::duration<Value, std::ratio<1>>> {
    static std::string convert(const std::chrono::duration<Value, std::ratio<1>>& duration) {
        ReusableStringStream rss;
...
struct StringMaker<std::chrono::duration<Value, std::ratio<60>>> {
    static std::string convert(const std::chrono::duration<Value, std::ratio<60>>& duration) {
        ReusableStringStream rss;
...
struct StringMaker<std::chrono::duration<Value, std::ratio<3600>>> {
    static std::string convert(const std::chrono::duration<Value, std::ratio<3600>>& duration) {
        ReusableStringStream rss;
...
struct StringMaker<std::chrono::time_point<Clock, Duration>> {
    static std::string convert(const std::chrono::time_point<Clock, Duration>& time_point) {
...
void formatReconstructedExpression(
    std::ostream& os, const std::string& lhs, StringRef op, const std::string& rhs);
...
class BinaryExpr : public ITransientExpression {
  ...
  public:
    ...
    template<typename T>
    auto operator&&(T) const -> const BinaryExpr<LhsT, const RhsT&> {
        static_assert(
...
// deduces as an int)
template<typename LhsT, typename RhsT>
auto compareEqual(const LhsT& lhs, const RhsT& rhs) -> bool {
    return static_cast<bool>(lhs == rhs);
}
template<typename T>
auto compareEqual(T* const& lhs, int rhs) -> bool {
    return lhs == reinterpret_cast<const void*>(rhs);
}
template<typename T>
...
auto compareEqual(int lhs, T* const& rhs) -> bool {
    return reinterpret_cast<const void*>(lhs) == rhs;
}
template<typename T>
...
template<typename LhsT, typename RhsT>
auto compareNotEqual(const LhsT& lhs, RhsT&& rhs) -> bool {
    return static_cast<bool>(lhs != rhs);
}
template<typename T>
...
class ExprLhs {
  ...
  public:
    ...
    auto operator==(bool rhs) -> const BinaryExpr<LhsT, bool> {
    ...
    template<typename RhsT>
    auto operator&&(const RhsT&) -> const BinaryExpr<LhsT, const RhsT&> {
        static_assert(
...
void handleExpression(const ITransientExpression& expr);

template<typename T>
void handleExpression(const ExprLhs<T>& expr) {
    handleExpression(expr.makeUnaryExpr());
}

struct Decomposer {
    template<typename T>
    auto operator<=(const T& lhs) -> ExprLhs<const T&> {
...
struct IResultCapture {
    ...
    virtual bool sectionStarted(const SectionInfo& sectionInfo, Counts& assertions) = 0;
    virtual void sectionEnded(const SectionEndInfo& endInfo) = 0;
    virtual void sectionEndedEarly(const SectionEndInfo& endInfo) = 0;

    virtual auto acquireGeneratorTracker(StringRef generatorName, const SourceLineInfo& lineInfo)
        -> IGeneratorTracker& = 0;

#if defined(CATCH_CONFIG_ENABLE_BENCHMARKING)
    virtual void benchmarkPreparing(const std::string& name) = 0;
    virtual void benchmarkStarting(const BenchmarkInfo& info) = 0;
    virtual void benchmarkEnded(const BenchmarkStats<>& stats) = 0;
    virtual void benchmarkFailed(const std::string& error) = 0;
#endif  // CATCH_CONFIG_ENABLE_BENCHMARKING

    virtual void pushScopedMessage(const MessageInfo& message) = 0;
    virtual void popScopedMessage(const MessageInfo& message) = 0;

    virtual void emplaceUnscopedMessage(const MessageBuilder& builder) = 0;

    virtual void handleFatalErrorCondition(StringRef message) = 0;

    virtual void handleExpr(
        const AssertionInfo& info, const ITransientExpression& expr,
        AssertionReaction& reaction) = 0;
    virtual void handleMessage(
        const AssertionInfo& info, ResultWas::OfType resultType, const StringRef& message,
        AssertionReaction& reaction) = 0;
    virtual void handleUnexpectedExceptionNotThrown(
        const AssertionInfo& info, AssertionReaction& reaction) = 0;
    virtual void handleUnexpectedInflightException(
        const AssertionInfo& info, const std::string& message, AssertionReaction& reaction) = 0;
    virtual void handleIncomplete(const AssertionInfo& info) = 0;
    virtual void handleNonExpr(
        const AssertionInfo& info, ResultWas::OfType resultType, AssertionReaction& reaction) = 0;
...
class LazyExpression {
    ...
    const ITransientExpression* m_transientExpression = nullptr;
    bool m_isNegated;

  public:
    LazyExpression(bool isNegated);
    LazyExpression(const LazyExpression& other);
    LazyExpression& operator=(const LazyExpression&) = delete;

    explicit operator bool() const;

    friend auto operator<<(std::ostream& os, const LazyExpression& lazyExpr) -> std::ostream&;
};
...
class AssertionHandler {
  ...
  public:
    AssertionHandler(
        const StringRef& macroName, const SourceLineInfo& lineInfo, StringRef capturedExpression,
        ResultDisposition::Flags resultDisposition);
    ~AssertionHandler() {
        if (!m_completed) {
    ...
    template<typename T>
    void handleExpr(const ExprLhs<T>& expr) {
        handleExpr(expr.makeUnaryExpr());
    }
    void handleExpr(const ITransientExpression& expr);

    void handleMessage(ResultWas::OfType resultType, const StringRef& message);
...
void handleExceptionMatchExpr(
    AssertionHandler& handler, const std::string& str, const StringRef& matcherString);
...
struct MessageInfo {
    MessageInfo(
        const StringRef& _macroName, const SourceLineInfo& _lineInfo, ResultWas::OfType _type);
...
struct MessageBuilder : MessageStream {
    MessageBuilder(
        const StringRef& macroName, const SourceLineInfo& lineInfo, ResultWas::OfType type);
...
class ScopedMessage {
  public:
    explicit ScopedMessage(const MessageBuilder& builder);
...
class Capturer {
  ...
  public:
    Capturer(
        StringRef macroName, const SourceLineInfo& lineInfo, ResultWas::OfType resultType,
        StringRef names);
    ~Capturer();
    ...
    template<typename T>
    void captureValues(size_t index, const T& value) {
        captureValue(index, Catch::Detail::stringify(value));
    }

    template<typename T, typename... Ts>
    void captureValues(size_t index, const T& value, const Ts&... values) {
        captureValue(index, Catch::Detail::stringify(value));
...
struct SectionInfo {
    SectionInfo(const SourceLineInfo& _lineInfo, const std::string& _name);

    // Deprecated
    SectionInfo(const SourceLineInfo& _lineInfo, const std::string& _name, const std::string&) :
...
class Section : NonCopyable {
  public:
    Section(const SectionInfo& info);
    ~Section();
...
struct IRegistryHub {
    ...
    virtual const ITagAliasRegistry& getTagAliasRegistry() const = 0;
    virtual const IExceptionTranslatorRegistry& getExceptionTranslatorRegistry() const = 0;

    virtual const StartupExceptionRegistry& getStartupExceptionRegistry() const = 0;
};

struct IMutableRegistryHub {
    virtual ~IMutableRegistryHub();
    virtual void registerReporter(const std::string& name, const IReporterFactoryPtr& factory) = 0;
    virtual void registerListener(const IReporterFactoryPtr& factory) = 0;
    virtual void registerTest(const TestCase& testInfo) = 0;
    virtual void registerTranslator(const IExceptionTranslator* translator) = 0;
    virtual void registerTagAlias(
        const std::string& alias, const std::string& tag, const SourceLineInfo& lineInfo) = 0;
    virtual void registerStartupException() noexcept = 0;
    virtual IMutableEnumValuesRegistry& getMutableEnumValuesRegistry() = 0;
};

const IRegistryHub& getRegistryHub();
...
struct IExceptionTranslator;
using ExceptionTranslators = std::vector<std::unique_ptr<const IExceptionTranslator>>;
...
class Approx {
  ...
  public:
    ...
    Approx operator()(const T& value) const {
    ...
    template<
        typename T,
        typename = typename std::enable_if<std::is_constructible<double, T>::value>::type>
    explicit Approx(const T& value) : Approx(static_cast<double>(value)) {}
    ...
    template<
        typename T,
        typename = typename std::enable_if<std::is_constructible<double, T>::value>::type>
    Approx& epsilon(const T& newEpsilon) {
    ...
    template<
        typename T,
        typename = typename std::enable_if<std::is_constructible<double, T>::value>::type>
    Approx& margin(const T& newMargin) {
...
struct StringMaker<Catch::Detail::Approx> {
    static std::string convert(const Catch::Detail::Approx& value);
};
...
bool replaceInPlace(std::string& str, const std::string& replaceThis, const std::string& withThis);
...
struct MatcherMethod {
    virtual bool match(const ObjectT& arg) const = 0;
};
...
struct MatcherBase : MatcherUntypedBase, MatcherMethod<T> {
    MatchAllOf<T> operator&&(const MatcherBase& other) const;
    MatchAnyOf<T> operator||(const MatcherBase& other) const;
    MatchNotOf<T> operator!() const;
};

template<typename ArgT>
struct MatchAllOf : MatcherBase<ArgT> {
    bool match(const ArgT& arg) const override {
    ...
    MatchAllOf<ArgT> operator&&(const MatcherBase<ArgT>& other) {
        auto copy(*this);
...
struct MatchAnyOf : MatcherBase<ArgT> {
    ...
    MatchAnyOf<ArgT> operator||(const MatcherBase<ArgT>& other) {
        auto copy(*this);
        copy.m_matchers.push_back(&other);
        return copy;
    }
...
struct MatchNotOf : MatcherBase<ArgT> {
    MatchNotOf(const MatcherBase<ArgT>& underlyingMatcher) :
        m_underlyingMatcher(underlyingMatcher) {}

    bool match(const ArgT& arg) const override { return !m_underlyingMatcher.match(arg); }
...
template<typename T>
MatchAllOf<T> MatcherBase<T>::operator&&(const MatcherBase& other) const {
    return MatchAllOf<T>() && *this && other;
}
template<typename T>
MatchAnyOf<T> MatcherBase<T>::operator||(const MatcherBase& other) const {
...
class ExceptionMessageMatcher : public MatcherBase<std::exception> {
  ...
  public:
    ExceptionMessageMatcher(const std::string& message) : m_message(message) {}
...
Exception::ExceptionMessageMatcher Message(const std::string& message);
...
class PredicateMatcher : public MatcherBase<T> {
  ...
  public:
    PredicateMatcher(const std::function<bool(const T&)>& elem, const std::string& descr) :
        m_predicate(std::move(elem)), m_description(Detail::finalizeDescription(descr)) {}

    bool match(const T& item) const override { return m_predicate(item); }
...
Generic::PredicateMatcher<T> Predicate(
    const std::function<bool(const T&)>& predicate, const std::string& description = "") {
...
struct CasedString {
    CasedString(const std::string& str, CaseSensitive::Choice caseSensitivity);
    std::string adjustString(const std::string& str) const;
...
struct StringMatcherBase : MatcherBase<std::string> {
    StringMatcherBase(const std::string& operation, const CasedString& comparator);
    std::string describe() const override;
...
struct EqualsMatcher : StringMatcherBase {
    EqualsMatcher(const CasedString& comparator);
    bool match(const std::string& source) const override;
};
struct ContainsMatcher : StringMatcherBase {
    ContainsMatcher(const CasedString& comparator);
    bool match(const std::string& source) const override;
};
struct StartsWithMatcher : StringMatcherBase {
    StartsWithMatcher(const CasedString& comparator);
    bool match(const std::string& source) const override;
};
struct EndsWithMatcher : StringMatcherBase {
    EndsWithMatcher(const CasedString& comparator);
    bool match(const std::string& source) const override;
};

struct RegexMatcher : MatcherBase<std::string> {
    RegexMatcher(std::string regex, CaseSensitive::Choice caseSensitivity);
    bool match(const std::string& matchee) const override;
    std::string describe() const override;
...
StdString::EqualsMatcher Equals(
    const std::string& str, CaseSensitive::Choice caseSensitivity = CaseSensitive::Yes);
...
StdString::RegexMatcher Matches(
    const std::string& regex, CaseSensitive::Choice caseSensitivity = CaseSensitive::Yes);
...
struct ContainsElementMatcher : MatcherBase<std::vector<T, Alloc>> {
    ContainsElementMatcher(const T& comparator) : m_comparator(comparator) {}

    bool match(const std::vector<T, Alloc>& v) const override {
        for (const auto& el : v) {
            if (el == m_comparator) {
                return true;
            }
...
struct ContainsMatcher : MatcherBase<std::vector<T, AllocMatch>> {
    ContainsMatcher(const std::vector<T, AllocComp>& comparator) : m_comparator(comparator) {}

    bool match(const std::vector<T, AllocMatch>& v) const override {
        // !TBD: see note in EqualsMatcher
        if (m_comparator.size() > v.size()) return false;
        for (const auto& comparator : m_comparator) {
            auto present = false;
            for (const auto& el : v) {
                if (el == comparator) {
                    present = true;
    ...
    const std::vector<T, AllocComp>& m_comparator;
};

template<typename T, typename AllocComp, typename AllocMatch>
struct EqualsMatcher : MatcherBase<std::vector<T, AllocMatch>> {
    EqualsMatcher(const std::vector<T, AllocComp>& comparator) : m_comparator(comparator) {}
...
struct ApproxMatcher : MatcherBase<std::vector<T, AllocMatch>> {
    ApproxMatcher(const std::vector<T, AllocComp>& comparator) : m_comparator(comparator) {}
    ...
    ApproxMatcher& epsilon(const T& newEpsilon) {
        approx.epsilon(newEpsilon);
        return *this;
    }
    template<typename = typename std::enable_if<std::is_constructible<double, T>::value>::type>
    ApproxMatcher& margin(const T& newMargin) {
        approx.margin(newMargin);
        return *this;
    }
    template<typename = typename std::enable_if<std::is_constructible<double, T>::value>::type>
    ApproxMatcher& scale(const T& newScale) {
...
struct UnorderedEqualsMatcher : MatcherBase<std::vector<T, AllocMatch>> {
    UnorderedEqualsMatcher(const std::vector<T, AllocComp>& target) : m_target(target) {}
    bool match(const std::vector<T, AllocMatch>& vec) const override {
  ...
  private:
    const std::vector<T, AllocComp>& m_target;
};
...
Vector::ContainsMatcher<T, AllocComp, AllocMatch> Contains(
    const std::vector<T, AllocComp>& comparator) {
    return Vector::ContainsMatcher<T, AllocComp, AllocMatch>(comparator);
}

template<typename T, typename Alloc = std::allocator<T>>
Vector::ContainsElementMatcher<T, Alloc> VectorContains(const T& comparator) {
...
Vector::UnorderedEqualsMatcher<T, AllocComp, AllocMatch> UnorderedEquals(
    const std::vector<T, AllocComp>& target) {
...
class MatchExpr : public ITransientExpression {
  ...
  public:
    MatchExpr(const ArgT& arg, const MatcherT& matcher, const StringRef& matcherString) :
...
void handleExceptionMatchExpr(
    AssertionHandler& handler, const StringMatcher& matcher, const StringRef& matcherString);

template<typename ArgT, typename MatcherT>
auto makeMatchExpr(const ArgT& arg, const MatcherT& matcher, const StringRef& matcherString)
...
struct IGeneratorTracker {
    ...
    virtual auto getGenerator() const -> const Generators::GeneratorBasePtr& = 0;
...
[[noreturn]] void throw_exception(const Ex& e) {
    throw e;
}
...
[[noreturn]] void throw_domain_error(const std::string& msg);
...
class GeneratorWrapper final {
  ...
  public:
    ...
    const T& get() const { return m_generator->get(); }
...
class Generators : public IGenerator<T> {
  ...
  public:
    ...
    const T& get() const override { return m_generators[m_current].get(); }
...
auto acquireGeneratorTracker(StringRef generatorName, const SourceLineInfo& lineInfo)
    -> IGeneratorTracker&;
...
//       return type. Yeah.
auto generate(StringRef generatorName, const SourceLineInfo& lineInfo, const L& generatorExpression)
    ...
    const auto& generator = static_cast<const IGenerator<UnderlyingType>&>(*tracker.getGenerator());
...
class TakeGenerator : public IGenerator<T> {
  ...
  public:
    ...
    const T& get() const override { return m_generator.get(); }
    bool next() override {
        ++m_returned;
...
class MapGenerator : public IGenerator<T> {
  ...
  public:
    ...
    const T& get() const override { return m_cache; }
    bool next() override {
        const auto success = m_generator.next();
        if (success) {
            m_cache = m_function(m_generator.get());
...
class ChunkGenerator final : public IGenerator<std::vector<T>> {
  ...
  public:
    ...
    const std::vector<T>& get() const override { return m_chunk; }
    bool next() override {
        m_chunk.clear();
...
using IConfigPtr = std::shared_ptr<const IConfig>;
...
struct IContext {
    ...
    virtual const IConfigPtr& getConfig() const = 0;
};
...
struct IMutableContext : IContext {
    ...
    virtual void setConfig(const IConfigPtr& config) = 0;
...
class Option {
  public:
    ...
    Option(const T& _value) : nullableValue(new(storage) T(_value)) {}
    Option(const Option& _other) : nullableValue(_other ? new(storage) T(*_other) : nullptr) {}
    ...
    T valueOr(const T& defaultValue) const { return nullableValue ? *nullableValue : defaultValue; }
...
struct IConfig : NonCopyable {
    ...
    virtual const TestSpec& testSpec() const = 0;
    virtual bool hasTestFilters() const = 0;
    virtual const std::vector<std::string>& getTestsOrTags() const = 0;
    virtual RunTests::InWhatOrder runOrder() const = 0;
    virtual unsigned int rngSeed() const = 0;
    virtual UseColour::YesOrNo useColour() const = 0;
    virtual const std::vector<std::string>& getSectionsToRun() const = 0;
...
class SimplePcg32 {
  ...
  private:
    friend bool operator==(const SimplePcg32& lhs, const SimplePcg32& rhs);
    friend bool operator!=(const SimplePcg32& lhs, const SimplePcg32& rhs);
...
class RandomFloatingGenerator final : public IGenerator<Float> {
  ...
  public:
    ...
    const Float& get() const override { return m_current_number; }
    bool next() override {
...
class RandomIntegerGenerator final : public IGenerator<Integer> {
  ...
  public:
    ...
    const Integer& get() const override { return m_current_number; }
    bool next() override {
        m_current_number = m_dist(m_rng);
        return true;
    }
...
template<typename T>
typename std::enable_if<
    std::is_integral<T>::value && !std::is_same<T, bool>::value, GeneratorWrapper<T>>::type
random(T a, T b) {
    return GeneratorWrapper<T>(pf::make_unique<RandomIntegerGenerator<T>>(a, b));
}

template<typename T>
typename std::enable_if<std::is_floating_point<T>::value, GeneratorWrapper<T>>::type random(
    T a, T b) {
...
class RangeGenerator final : public IGenerator<T> {
  ...
  public:
    RangeGenerator(const T& start, const T& end, const T& step) :
    ...
    RangeGenerator(const T& start, const T& end) :
...
template<typename T>
GeneratorWrapper<T> range(const T& start, const T& end, const T& step) {
    static_assert(
        std::is_arithmetic<T>::value && !std::is_same<T, bool>::value, "Type must be numeric");
    return GeneratorWrapper<T>(pf::make_unique<RangeGenerator<T>>(start, end, step));
}

template<typename T>
GeneratorWrapper<T> range(const T& start, const T& end) {
    static_assert(
        std::is_integral<T>::value && !std::is_same<T, bool>::value, "Type must be an integer");
...
class IteratorGenerator final : public IGenerator<T> {
  ...
  public:
    ...
    const T& get() const override { return m_elems[m_current]; }
...
GeneratorWrapper<ResultType> from_range(const Container& cnt) {
...
struct TestCaseInfo {
    ...
    TestCaseInfo(
        const std::string& _name, const std::string& _className, const std::string& _description,
        const std::vector<std::string>& _tags, const SourceLineInfo& _lineInfo);
...
TestCase makeTestCase(
    ITestInvoker* testCase, const std::string& className, const NameAndTags& nameAndTags,
...
inline std::string getAnnotation(
    Class cls, const std::string& annotationName, const std::string& testCaseName) {
...
struct StringHolder : MatcherBase<NSString*> {
    ...
    StringHolder(const StringHolder& other) : m_substr([other.m_substr copy]) {}
...
class WildcardPattern {
  ...
  public:
    WildcardPattern(const std::string& pattern, CaseSensitive::Choice caseSensitivity);
    virtual ~WildcardPattern() = default;
    virtual bool matches(const std::string& str) const;

  private:
    std::string normaliseString(const std::string& str) const;
    CaseSensitive::Choice m_caseSensitivity;
...
class TestSpec {
    class Pattern {
      public:
        ...
        virtual ~Pattern();
        virtual bool matches(const TestCaseInfo& testCase) const = 0;
    ...
    class NamePattern : public Pattern {
      public:
        explicit NamePattern(const std::string& name, const std::string& filterString);
        bool matches(const TestCaseInfo& testCase) const override;
    ...
    class TagPattern : public Pattern {
      public:
        explicit TagPattern(const std::string& tag, const std::string& filterString);
        bool matches(const TestCaseInfo& testCase) const override;
    ...
    class ExcludedPattern : public Pattern {
      public:
        explicit ExcludedPattern(const PatternPtr& underlyingPattern);
        bool matches(const TestCaseInfo& testCase) const override;
    ...
    struct Filter {
        ...
        bool matches(const TestCaseInfo& testCase) const;
  ...
  public:
    ...
    bool matches(const TestCaseInfo& testCase) const;
    Matches matchesByFilter(const std::vector<TestCase>& testCases, const IConfig& config) const;
...
struct ITagAliasRegistry {
    ...
    virtual const TagAlias* find(const std::string& alias) const = 0;
    virtual std::string expandAliases(const std::string& unexpandedTestSpec) const = 0;

    static const ITagAliasRegistry& get();
};
...
class TestSpecParser {
    ...
    TestSpec m_testSpec;
    const ITagAliasRegistry* m_tagAliases = nullptr;

  public:
    TestSpecParser(const ITagAliasRegistry& tagAliases);

    TestSpecParser& parse(const std::string& arg);
    TestSpec testSpec();
...
TestSpec parseTestSpec(const std::string& arg);
...
class Config : public IConfig {
  public:
    ...
    const TestSpec& testSpec() const override;
  ...
  private:
    const IStream* openStream();
    ConfigData m_data;

    std::unique_ptr<const IStream> m_stream;
    TestSpec m_testSpec;
...
struct AssertionResultData {
    ...
    AssertionResultData(ResultWas::OfType _resultType, const LazyExpression& _lazyExpression);
...
struct ReporterConfig {
    explicit ReporterConfig(const IConfigPtr& _fullConfig);

    ReporterConfig(const IConfigPtr& _fullConfig, std::ostream& _stream);
...
struct LazyStat : Option<T> {
    LazyStat& operator=(const T& _value) {
...
struct TestRunInfo {
    TestRunInfo(const std::string& _name);
    std::string name;
};
struct GroupInfo {
    GroupInfo(const std::string& _name, std::size_t _groupIndex, std::size_t _groupsCount);
...
struct AssertionStats {
    AssertionStats(
        const AssertionResult& _assertionResult, const std::vector<MessageInfo>& _infoMessages,
        const Totals& _totals);

    AssertionStats(const AssertionStats&) = default;
    AssertionStats(AssertionStats&&) = default;
    AssertionStats& operator=(const AssertionStats&) = delete;
...
struct SectionStats {
    SectionStats(
        const SectionInfo& _sectionInfo, const Counts& _assertions, double _durationInSeconds,
        bool _missingAssertions);
    SectionStats(const SectionStats&) = default;
    SectionStats(SectionStats&&) = default;
    SectionStats& operator=(const SectionStats&) = default;
...
struct TestCaseStats {
    TestCaseStats(
        const TestCaseInfo& _testInfo, const Totals& _totals, const std::string& _stdOut,
        const std::string& _stdErr, bool _aborting);

    TestCaseStats(const TestCaseStats&) = default;
    TestCaseStats(TestCaseStats&&) = default;
    TestCaseStats& operator=(const TestCaseStats&) = default;
...
struct TestGroupStats {
    TestGroupStats(const GroupInfo& _groupInfo, const Totals& _totals, bool _aborting);
    TestGroupStats(const GroupInfo& _groupInfo);

    TestGroupStats(const TestGroupStats&) = default;
    TestGroupStats(TestGroupStats&&) = default;
    TestGroupStats& operator=(const TestGroupStats&) = default;
...
struct TestRunStats {
    TestRunStats(const TestRunInfo& _runInfo, const Totals& _totals, bool _aborting);

    TestRunStats(const TestRunStats&) = default;
    TestRunStats(TestRunStats&&) = default;
    TestRunStats& operator=(const TestRunStats&) = default;
...
struct BenchmarkStats {
    ...
    operator BenchmarkStats<Duration2>() const {
        ...
        std::transform(
            samples.begin(), samples.end(), std::back_inserter(samples2), [](Duration d) {
                return Duration2(d);
            });
        return {
            info,
...
struct IStreamingReporter {
    ...
    virtual void noMatchingTestCases(const std::string& spec) = 0;

    virtual void reportInvalidArguments(const std::string&) {}

    virtual void testRunStarting(const TestRunInfo& testRunInfo) = 0;
    virtual void testGroupStarting(const GroupInfo& groupInfo) = 0;

    virtual void testCaseStarting(const TestCaseInfo& testInfo) = 0;
    virtual void sectionStarting(const SectionInfo& sectionInfo) = 0;

#if defined(CATCH_CONFIG_ENABLE_BENCHMARKING)
    virtual void benchmarkPreparing(const std::string&) {}
    virtual void benchmarkStarting(const BenchmarkInfo&) {}
    virtual void benchmarkEnded(const BenchmarkStats<>&) {}
    virtual void benchmarkFailed(const std::string&) {}
#endif  // CATCH_CONFIG_ENABLE_BENCHMARKING

    virtual void assertionStarting(const AssertionInfo& assertionInfo) = 0;

    // The return value indicates if the messages buffer should be cleared:
    virtual bool assertionEnded(const AssertionStats& assertionStats) = 0;

    virtual void sectionEnded(const SectionStats& sectionStats) = 0;
    virtual void testCaseEnded(const TestCaseStats& testCaseStats) = 0;
    virtual void testGroupEnded(const TestGroupStats& testGroupStats) = 0;
    virtual void testRunEnded(const TestRunStats& testRunStats) = 0;

    virtual void skipTest(const TestCaseInfo& testInfo) = 0;
...
struct IReporterFactory {
    ...
    virtual IStreamingReporterPtr create(const ReporterConfig& config) const = 0;
...
struct IReporterRegistry {
    ...
    virtual IStreamingReporterPtr create(
        const std::string& name, const IConfigPtr& config) const = 0;
    virtual const FactoryMap& getFactories() const = 0;
    virtual const Listeners& getListeners() const = 0;
};
...
bool shouldShowDuration(const IConfig& config, double duration);

std::string serializeFilters(const std::vector<std::string>& container);

template<typename DerivedT>
struct StreamingReporterBase : IStreamingReporter {
    StreamingReporterBase(const ReporterConfig& _config) :
    ...
    void reportInvalidArguments(const std::string&) override {}

    void testRunStarting(const TestRunInfo& _testRunInfo) override {
        currentTestRunInfo = _testRunInfo;
    }

    void testGroupStarting(const GroupInfo& _groupInfo) override { currentGroupInfo = _groupInfo; }

    void testCaseStarting(const TestCaseInfo& _testInfo) override {
    ...
    void testCaseEnded(const TestCaseStats& /* _testCaseStats */) override {
        currentTestCaseInfo.reset();
    }
    void testGroupEnded(const TestGroupStats& /* _testGroupStats */) override {
        currentGroupInfo.reset();
    }
    void testRunEnded(const TestRunStats& /* _testRunStats */) override {
        currentTestCaseInfo.reset();
        currentGroupInfo.reset();
...
struct CumulativeReporterBase : IStreamingReporter {
    ...
    struct SectionNode {
        explicit SectionNode(const SectionStats& _stats) : stats(_stats) {}
        virtual ~SectionNode() = default;

        bool operator==(const SectionNode& other) const {
            return stats.sectionInfo.lineInfo == other.stats.sectionInfo.lineInfo;
        }
        bool operator==(const std::shared_ptr<SectionNode>& other) const {
    ...
    struct BySectionInfo {
        BySectionInfo(const SectionInfo& other) : m_other(other) {}
        BySectionInfo(const BySectionInfo& other) : m_other(other.m_other) {}
        bool operator()(const std::shared_ptr<SectionNode>& node) const {
            return (
                (node->stats.sectionInfo.name == m_other.name)
                && (node->stats.sectionInfo.lineInfo == m_other.lineInfo));
        }
        void operator=(const BySectionInfo&) = delete;

      private:
        const SectionInfo& m_other;
    };
    ...
    CumulativeReporterBase(const ReporterConfig& _config) :
        m_config(_config.fullConfig()), stream(_config.stream()) {
        m_reporterPrefs.shouldRedirectStdOut = false;
        if (!DerivedT::getSupportedVerbosities().count(m_config->verbosity()))
            CATCH_ERROR("Verbosity level not supported by this reporter");
...
struct TestEventListenerBase : StreamingReporterBase<TestEventListenerBase> {
    TestEventListenerBase(const ReporterConfig& _config);

    static std::set<Verbosity> getSupportedVerbosities();

    void assertionStarting(const AssertionInfo&) override;
    bool assertionEnded(const AssertionStats&) override;
};
...
std::ostream& operator<<(std::ostream& os, const Colour&);
...
class ReporterRegistrar {
    class ReporterFactory : public IReporterFactory {
        IStreamingReporterPtr create(const ReporterConfig& config) const override {
  ...
  public:
    explicit ReporterRegistrar(const std::string& name) {
...
struct CompactReporter : StreamingReporterBase<CompactReporter> {
    ...
    bool assertionEnded(const AssertionStats& _assertionStats) override;
...
struct ConsoleReporter : StreamingReporterBase<ConsoleReporter> {
    ...
    ConsoleReporter(const ReporterConfig& config);
    ~ConsoleReporter() override;
    static std::string getDescription();

    void noMatchingTestCases(const std::string& spec) override;

    void reportInvalidArguments(const std::string& arg) override;
...
#if defined(CATCH_CONFIG_ENABLE_BENCHMARKING)
    void benchmarkPreparing(const std::string& name) override;
    void benchmarkStarting(const BenchmarkInfo& info) override;
  ...
  private:
    ...
    void printClosedHeader(const std::string& _name);
    void printOpenHeader(const std::string& _name);

    // if string has a : in first line will set indent to follow it on
    // subsequent lines
    void printHeaderString(const std::string& _string, std::size_t indent = 0);

    void printTotals(const Totals& totals);
    void printSummaryRow(
        const std::string& label, const std::vector<SummaryColumn>& cols, std::size_t row);
...
class XmlWriter {
  public:
    ...
    void writeStylesheetRef(const std::string& url);
...
class JunitReporter : public CumulativeReporterBase<JunitReporter> {
  public:
    JunitReporter(const ReporterConfig& _config);
    ...
    void writeSection(
        const std::string& className, const std::string& rootName, const SectionNode& sectionNode,
        bool testOkToFail);
...
class XmlReporter : public StreamingReporterBase<XmlReporter> {
  public:
    XmlReporter(const ReporterConfig& _config);
  ...
  public:  // StreamingReporterBase
    void noMatchingTestCases(const std::string& s) override;
...
inline void keep_memory(T* p) {
    // thanks @milleniumbug
    *reinterpret_cast<volatile char*>(p) = *reinterpret_cast<const volatile char*>(p);
}
...
struct BenchmarkFunction {
  ...
  public:
    ...
    BenchmarkFunction(const BenchmarkFunction& that) : f(that.f->clone()) {}
    ...
    BenchmarkFunction& operator=(const BenchmarkFunction& that) {
...
double weighted_average_quantile(
    int k, int q, std::vector<double>::iterator first, std::vector<double>::iterator last);
...
sample resample(URng& rng, int resamples, Iterator first, Iterator last, Estimator& estimator) {
    ...
    std::generate_n(std::back_inserter(out), resamples, [n, first, &estimator, &dist, &rng] {
        ...
        resampled.reserve(n);
        std::generate_n(
            std::back_inserter(resampled), n, [first, &dist, &rng] { return first[dist(rng)]; });
...
Estimate<double> bootstrap(
    double confidence_level, Iterator first, Iterator last, const sample& resample,
    Estimator&& estimator) {
...
bootstrap_analysis analyse_samples(
    double confidence_level, int n_resamples, std::vector<double>::iterator first,
...
SampleAnalysis<Duration> analyse(
    ...
    if (!cfg.benchmarkNoAnalysis()) {
        ...
        std::transform(
            first, last, std::back_inserter(samples), [](Duration d) { return d.count(); });
...
struct ObjectStorage {
  ...
  private:
    ...
    const T& stored_object() const { return *static_cast<T*>(static_cast<void*>(&data)); }
...
struct NameAndLocation {
    ...
    NameAndLocation(const std::string& _name, const SourceLineInfo& _location);
    friend bool operator==(const NameAndLocation& lhs, const NameAndLocation& rhs) {
...
class ITracker {
  ...
  public:
    ITracker(const NameAndLocation& nameAndLoc) : m_nameAndLocation(nameAndLoc) {}

    // static queries
    const NameAndLocation& nameAndLocation() const { return m_nameAndLocation; }
    ...
    virtual void addChild(const ITrackerPtr& child) = 0;
    virtual ITrackerPtr findChild(const NameAndLocation& nameAndLocation) = 0;
...
class TrackerBase : public ITracker {
  ...
  public:
    TrackerBase(const NameAndLocation& nameAndLocation, TrackerContext& ctx, ITracker* parent);
    ...
    void addChild(const ITrackerPtr& child) override;

    ITrackerPtr findChild(const NameAndLocation& nameAndLocation) override;
...
class SectionTracker : public TrackerBase {
  ...
  public:
    SectionTracker(const NameAndLocation& nameAndLocation, TrackerContext& ctx, ITracker* parent);

    bool isSectionTracker() const override;

    bool isComplete() const override;

    static SectionTracker& acquire(TrackerContext& ctx, const NameAndLocation& nameAndLocation);

    void tryOpen();

    void addInitialFilters(const std::vector<std::string>& filters);
    void addNextFilters(const std::vector<std::string>& filters);
    //! Returns filters active in this tracker
    const std::vector<std::string>& getFilters() const;
    //! Returns whitespace-trimmed name of the tracked section
    const std::string& trimmedName() const;
};
...
std::string StringMaker<Catch::Detail::Approx>::convert(const Catch::Detail::Approx& value) {
...
class RunContext : public IResultCapture, public IRunner {
  public:
    ...
    explicit RunContext(const IConfigPtr& _config, IStreamingReporterPtr&& reporter);

    ~RunContext() override;

    void testGroupStarting(
        const std::string& testSpec, std::size_t groupIndex, std::size_t groupsCount);
    void testGroupEnded(
        const std::string& testSpec, const Totals& totals, std::size_t groupIndex,
  ...
  public:  // IResultCapture
    ...
    void handleMessage(
        ...
        AssertionReaction& reaction) override;
    void handleUnexpectedExceptionNotThrown(
        const AssertionInfo& info, AssertionReaction& reaction) override;
    void handleUnexpectedInflightException(
        const AssertionInfo& info, const std::string& message,
        AssertionReaction& reaction) override;
    void handleIncomplete(const AssertionInfo& info) override;
    void handleNonExpr(
        const AssertionInfo& info, ResultWas::OfType resultType,
        AssertionReaction& reaction) override;

    bool sectionStarted(const SectionInfo& sectionInfo, Counts& assertions) override;

    void sectionEnded(const SectionEndInfo& endInfo) override;
    void sectionEndedEarly(const SectionEndInfo& endInfo) override;
    ...
    void pushScopedMessage(const MessageInfo& message) override;
    void popScopedMessage(const MessageInfo& message) override;

    void emplaceUnscopedMessage(const MessageBuilder& builder) override;
  ...
  private:
    ...
    void assertionEnded(const AssertionResult& result);
    void reportExpr(
        const AssertionInfo& info, ResultWas::OfType resultType, const ITransientExpression* expr,
        bool negated);
  ...
  private:
    ...
    const TestCase* m_activeTestCase = nullptr;
...
void seedRng(const IConfig& config);
unsigned int rngSeed();
}  // end namespace Catch
...
namespace {
auto operator<<(std::ostream& os, const ITransientExpression& expr) -> std::ostream& {
...
LazyExpression::LazyExpression(const LazyExpression& other) : m_isNegated(other.m_isNegated) {}

LazyExpression::operator bool() const { return m_transientExpression != nullptr; }

auto operator<<(std::ostream& os, const LazyExpression& lazyExpr) -> std::ostream& {
...
void AssertionHandler::handleExpr(const ITransientExpression& expr) {
    m_resultCapture.handleExpr(m_assertionInfo, expr, m_reaction);
}
void AssertionHandler::handleMessage(ResultWas::OfType resultType, const StringRef& message) {
...
AssertionResultData::AssertionResultData(
    ResultWas::OfType _resultType, const LazyExpression& _lazyExpression) :
...
void handleExceptionMatchExpr(
    ...
    MatchExpr<std::string, const StringMatcher&> expr(exceptionMessage, matcher, matcherString);
...
class Column {
  ...
  public:
    class iterator {
        ...
        iterator(const Column& column, size_t stringIndex) :
            m_column(column), m_stringIndex(stringIndex) {}

        auto line() const -> const std::string& { return m_column.m_strings[m_stringIndex]; }
        ...
        auto addIndentAndSuffix(const std::string& plain) const -> std::string {
      ...
      public:
        ...
        explicit iterator(const Column& column) : m_column(column) {
    ...
    explicit Column(const std::string& text) { m_strings.push_back(text); }
    ...
    inline friend std::ostream& operator<<(std::ostream& os, const Column& col) {
        bool first = true;
        for (auto line : col) {
            if (first)
                first = false;
    ...
    auto operator+(const Column& other) -> Columns;
...
class Columns {
  ...
  public:
    class iterator {
        ...
        iterator(const Columns& columns, EndTag) :
            m_columns(columns.m_columns), m_activeIterators(0) {
            m_iterators.reserve(m_columns.size());

            for (const auto& col : m_columns) m_iterators.push_back(col.end());
        }
      ...
      public:
        ...
        explicit iterator(const Columns& columns) :
            m_columns(columns.m_columns), m_activeIterators(m_columns.size()) {
            m_iterators.reserve(m_columns.size());

            for (const auto& col : m_columns) m_iterators.push_back(col.begin());
        }
    ...
    auto operator+=(const Column& col) -> Columns& {
    ...
    inline friend std::ostream& operator<<(std::ostream& os, const Columns& cols) {
        bool first = true;
        for (auto line : cols) {
            if (first)
                first = false;
...
inline auto Column::operator+(const Column& other) -> Columns {
    Columns cols;
    cols += *this;
    cols += other;
    return cols;
...
class Args {
  ...
  public:
    Args(int argc, const char* const* argv) : m_exeName(argv[0]), m_args(argv + 1, argv + argc) {}
...
class TokenStream {
  ...
  public:
    explicit TokenStream(const Args& args) : TokenStream(args.m_args.begin(), args.m_args.end()) {}
...
class ResultValueBase : public ResultBase {
  ...
  protected:
    ...
    ResultValueBase(const ResultValueBase& other) : ResultBase(other) {
        if (m_type == ResultBase::Ok) new (&m_value) T(other.m_value);
    }

    ResultValueBase(Type, const T& value) : ResultBase(Ok) { new (&m_value) T(value); }

    auto operator=(const ResultValueBase& other) -> ResultValueBase& {
...
class BasicResult : public ResultValueBase<T> {
  public:
    template<typename U>
    explicit BasicResult(const BasicResult<U>& other) :
        ResultValueBase<T>(other.type()), m_errorMessage(other.errorMessage()) {
        assert(type() != ResultBase::Ok);
    }

    template<typename U>
    static auto ok(const U& value) -> BasicResult {
        return {ResultBase::Ok, value};
    }
    static auto ok() -> BasicResult { return {ResultBase::Ok}; }
    static auto logicError(const std::string& message) -> BasicResult {
        return {ResultBase::LogicError, message};
    }
    static auto runtimeError(const std::string& message) -> BasicResult {
  ...
  protected:
    ...
    BasicResult(ResultBase::Type type, const std::string& message) :
...
class ParseState {
  public:
    ParseState(ParseResultType type, const TokenStream& remainingTokens) :
...
template<typename T>
inline auto convertInto(const std::string& source, T& target) -> ParserResult {
    std::stringstream ss;
    ss << source;
    ss >> target;
    if (ss.fail())
...
inline auto convertInto(const std::string& source, std::string& target) -> ParserResult {
    target = source;
    return ParserResult::ok(ParseResultType::Matched);
}
inline auto convertInto(const std::string& source, bool& target) -> ParserResult {
...
template<typename T>
inline auto convertInto(const std::string& source, CLARA_CONFIG_OPTIONAL_TYPE<T>& target)
    -> ParserResult {
    T temp;
...
struct BoundValueRefBase : BoundRef {
    virtual auto setValue(const std::string& arg) -> ParserResult = 0;
};
...
struct BoundValueRef : BoundValueRefBase {
    ...
    auto setValue(const std::string& arg) -> ParserResult override {
...
struct LambdaInvoker {
    ...
    static auto invoke(const L& lambda, const ArgType& arg) -> ParserResult {
        return lambda(arg);
    }
};
...
inline auto invokeLambda(const L& lambda, const std::string& arg) -> ParserResult {
    ArgType temp{};
    auto result = convertInto(arg, temp);
...
struct BoundLambda : BoundValueRefBase {
    ...
    explicit BoundLambda(const L& lambda) : m_lambda(lambda) {}
...
struct BoundFlagLambda : BoundFlagRefBase {
    ...
    static_assert(UnaryLambdaTraits<L>::isValid, "Supplied lambda must take exactly one argument");
    static_assert(
        std::is_same<typename UnaryLambdaTraits<L>::ArgType, bool>::value, "flags must be boolean");

    explicit BoundFlagLambda(const L& lambda) : m_lambda(lambda) {}
...
class ParserBase {
  public:
    ...
    virtual auto parse(const std::string& exeName, const TokenStream& tokens) const
        -> InternalParseResult = 0;
    virtual auto cardinality() const -> size_t { return 1; }

    auto parse(const Args& args) const -> InternalParseResult {
...
class ComposableParserImpl : public ParserBase {
  public:
    template<typename T>
    auto operator|(const T& other) const -> Parser;
...
class ParserRefImpl : public ComposableParserImpl<DerivedT> {
  protected:
    ...
    explicit ParserRefImpl(const std::shared_ptr<BoundRef>& ref) : m_ref(ref) {}

  public:
    template<typename T>
    ParserRefImpl(T& ref, const std::string& hint) :
        m_ref(std::make_shared<BoundValueRef<T>>(ref)), m_hint(hint) {}

    template<typename LambdaT>
    ParserRefImpl(const LambdaT& ref, const std::string& hint) :
        m_ref(std::make_shared<BoundLambda<LambdaT>>(ref)), m_hint(hint) {}

    auto operator()(const std::string& description) -> DerivedT& {
...
class ExeName : public ComposableParserImpl<ExeName> {
    ...
    template<typename LambdaT>
    static auto makeRef(const LambdaT& lambda) -> std::shared_ptr<BoundValueRefBase> {
  ...
  public:
    ...
    template<typename LambdaT>
    explicit ExeName(const LambdaT& lambda) : ExeName() {
        m_ref = std::make_shared<BoundLambda<LambdaT>>(lambda);
    }

    // The exe name is not parsed out of the normal tokens, but is handled specially
    auto parse(const std::string&, const TokenStream& tokens) const
        -> InternalParseResult override {
        return InternalParseResult::ok(ParseState(ParseResultType::NoMatch, tokens));
    }

    auto name() const -> std::string { return *m_name; }
    auto set(const std::string& newName) -> ParserResult {
...
class Arg : public ParserRefImpl<Arg> {
  public:
    ...
    auto parse(const std::string&, const TokenStream& tokens) const
        ...
        const auto& token = *remainingTokens;
...
inline auto normaliseOpt(const std::string& optName) -> std::string {
#ifdef CATCH_PLATFORM_WINDOWS
    if (optName[0] == '/')
...
class Opt : public ParserRefImpl<Opt> {
  ...
  public:
    template<typename LambdaT>
    explicit Opt(const LambdaT& ref) :
        ParserRefImpl(std::make_shared<BoundFlagLambda<LambdaT>>(ref)) {}

    explicit Opt(bool& ref) : ParserRefImpl(std::make_shared<BoundFlagRef>(ref)) {}

    template<typename LambdaT>
    Opt(const LambdaT& ref, const std::string& hint) : ParserRefImpl(ref, hint) {}

    template<typename T>
    Opt(T& ref, const std::string& hint) : ParserRefImpl(ref, hint) {}

    auto operator[](const std::string& optName) -> Opt& {
    ...
    auto isMatch(const std::string& optToken) const -> bool {
        ...
        if (remainingTokens && remainingTokens->type == TokenType::Option) {
            ...
            if (isMatch(token.token)) {
                if (m_ref->isFlag()) {
                    ...
                    const auto& argToken = *remainingTokens;
...
struct Parser : ParserBase {
    ...
    auto operator|=(const ExeName& exeName) -> Parser& {
        m_exeName = exeName;
        return *this;
    }

    auto operator|=(const Arg& arg) -> Parser& {
        m_args.push_back(arg);
        return *this;
    }

    auto operator|=(const Opt& opt) -> Parser& {
    ...
    template<typename T>
    auto operator+=(const T& other) -> Parser& {
        return operator|=(other);
    }
    template<typename T>
    auto operator+(const T& other) const -> Parser {
    ...
    void writeToStream(std::ostream& os) const {
        ...
        size_t optWidth = 0;
        for (const auto& cols : rows) optWidth = (std::max)(optWidth, cols.left.size() + 2);
    ...
    friend auto operator<<(std::ostream& os, const Parser& parser) -> std::ostream& {
    ...
    auto parse(const std::string& exeName, const TokenStream& tokens) const
        -> InternalParseResult override {
        struct ParserInfo {
            const ParserBase* parser = nullptr;
            size_t count = 0;
        };
        ...
        {
            size_t i = 0;
            for (const auto& opt : m_options) parseInfos[i++].parser = &opt;
            for (const auto& arg : m_args) parseInfos[i++].parser = &arg;
        }
...
template<typename DerivedT>
template<typename T>
auto ComposableParserImpl<DerivedT>::operator|(const T& other) const -> Parser {
    return Parser() | static_cast<const DerivedT&>(*this) | other;
}
}  // namespace detail
...
clara::Parser makeCommandLineParser(ConfigData& config) {
    ...
    const auto setWarning = [&](const std::string& warning) {
        auto warningSet = [&]() {
    ...
    const auto loadTestNamesFromFile = [&](const std::string& filename) {
    ...
    const auto setTestOrder = [&](const std::string& order) {
    ...
    const auto setRngSeed = [&](const std::string& seed) {
        if (seed != "time") return clara::detail::convertInto(seed, config.rngSeed);
        config.rngSeed = static_cast<unsigned int>(std::time(nullptr));
        return ParserResult::ok(ParseResultType::Matched);
    };
    const auto setColourUsage = [&](const std::string& useColour) {
    ...
    const auto setWaitForKeypress = [&](const std::string& keypress) {
    ...
    const auto setVerbosity = [&](const std::string& verbosity) {
    ...
    const auto setReporter = [&](const std::string& reporter) {
    ...
    auto cli =
        ...
        | Opt([&](int x) { config.abortAfter = x; }, "no. failures")["-x"]["--abortx"](
...
bool SourceLineInfo::operator==(const SourceLineInfo& other) const noexcept {
...
Config::Config(const ConfigData& data) : m_data(data), m_stream(openStream()) {
    ...
    if (!m_data.testsOrTags.empty()) {
        ...
        for (const auto& testOrTags : m_data.testsOrTags) {
...
const std::string& Config::getFilename() const { return m_data.outputFilename; }
...
const std::string& Config::getReporterName() const { return m_data.reporterName; }

const std::vector<std::string>& Config::getTestsOrTags() const { return m_data.testsOrTags; }
const std::vector<std::string>& Config::getSectionsToRun() const { return m_data.sectionsToRun; }

const TestSpec& Config::testSpec() const { return m_testSpec; }
...
const IStream* Config::openStream() { return Catch::makeStream(m_data.outputFilename); }
...
std::ostream& operator<<(std::ostream& os, const Colour&) { return os; }
...
class Context : public IMutableContext, NonCopyable {
  public:  // IContext
    ...
    const IConfigPtr& getConfig() const override { return m_config; }
  ...
  public:  // IMutableContext
    ...
    void setConfig(const IConfigPtr& config) override { m_config = config; }
...
namespace Catch {
void writeToDebugConsole(const std::string& text) { ::OutputDebugStringA(text.c_str()); }
}  // namespace Catch
...
std::unique_ptr<EnumInfo> makeEnumInfo(
    StringRef enumName, StringRef allValueNames, const std::vector<int>& values);

class EnumValuesRegistry : public IMutableEnumValuesRegistry {
    std::vector<std::unique_ptr<EnumInfo>> m_enumInfos;

    const EnumInfo& registerEnum(
        StringRef enumName, StringRef allEnums, const std::vector<int>& values) override;
};
...
StringRef EnumInfo::lookup(int value) const {
    for (const auto& valueToName : m_values) {
...
const EnumInfo& EnumValuesRegistry::registerEnum(
    StringRef enumName, StringRef allValueNames, const std::vector<int>& values) {
...
class ExceptionTranslatorRegistry : public IExceptionTranslatorRegistry {
  ...
  private:
    std::vector<std::unique_ptr<const IExceptionTranslator>> m_translators;
};
}  // namespace Catch
...
void reportFatal(const char* const message) {
...
class ListeningReporter : public IStreamingReporter {
  ...
  public:  // IStreamingReporter
...
#if defined(CATCH_CONFIG_ENABLE_BENCHMARKING)
    void benchmarkPreparing(const std::string& name) override;
    void benchmarkStarting(const BenchmarkInfo& benchmarkInfo) override;
    void benchmarkEnded(const BenchmarkStats<>& benchmarkStats) override;
    void benchmarkFailed(const std::string&) override;
#endif  // CATCH_CONFIG_ENABLE_BENCHMARKING

    void testRunStarting(const TestRunInfo& testRunInfo) override;
    void testGroupStarting(const GroupInfo& groupInfo) override;
    void testCaseStarting(const TestCaseInfo& testInfo) override;
    void sectionStarting(const SectionInfo& sectionInfo) override;
    void assertionStarting(const AssertionInfo& assertionInfo) override;
    ...
    void skipTest(const TestCaseInfo& testInfo) override;
...
ReporterConfig::ReporterConfig(const IConfigPtr& _fullConfig) :
    m_stream(&_fullConfig->stream()), m_fullConfig(_fullConfig) {}

ReporterConfig::ReporterConfig(const IConfigPtr& _fullConfig, std::ostream& _stream) :
    m_stream(&_stream), m_fullConfig(_fullConfig) {}

std::ostream& ReporterConfig::stream() const { return *m_stream; }
IConfigPtr ReporterConfig::fullConfig() const { return m_fullConfig; }

TestRunInfo::TestRunInfo(const std::string& _name) : name(_name) {}

GroupInfo::GroupInfo(const std::string& _name, std::size_t _groupIndex, std::size_t _groupsCount) :
...
AssertionStats::AssertionStats(
    ...
    if (assertionResult.hasMessage()) {
        ...
        MessageBuilder builder(
            assertionResult.getTestMacroName(),
...
TestGroupStats::TestGroupStats(const GroupInfo& _groupInfo, const Totals& _totals, bool _aborting) :
    groupInfo(_groupInfo), totals(_totals), aborting(_aborting) {}

TestGroupStats::TestGroupStats(const GroupInfo& _groupInfo) :
    groupInfo(_groupInfo), aborting(false) {}

TestGroupStats::~TestGroupStats() = default;

TestRunStats::TestRunStats(const TestRunInfo& _runInfo, const Totals& _totals, bool _aborting) :
...
std::size_t listTests(const Config& config);

std::size_t listTestsNamesOnly(const Config& config);
...
std::size_t listTags(const Config& config);

std::size_t listReporters();

Option<std::size_t> list(const std::shared_ptr<Config>& config);
...
std::size_t listTests(const Config& config) {
    const TestSpec& testSpec = config.testSpec();
    ...
    for (const auto& testCaseInfo : matchedTestCases) {
...
void TagInfo::add(const std::string& spelling) {
    ++count;
...
std::size_t listTags(const Config& config) {
    ...
    std::vector<TestCase> matchedTestCases =
        filterTests(getAllTestCasesSorted(config), testSpec, config);
    for (const auto& testCase : matchedTestCases) {
        for (const auto& tagName : testCase.getTestCaseInfo().tags) {
...
std::size_t listReporters() {
    ...
    const IReporterRegistry::FactoryMap& factories =
        getRegistryHub().getReporterRegistry().getFactories();
    std::size_t maxNameLen = 0;
    for (const auto& factoryKvp : factories)
...
bool ExceptionMessageMatcher::match(const std::exception& ex) const {
...
WithinAbsMatcher::WithinAbsMatcher(double target, double margin) :
    ...
    CATCH_ENFORCE(
        margin >= 0, "Invalid margin: " << margin << '.' << " Margin has to be non-negative.");
}

// Performs equivalent check of std::fabs(lhs - rhs) <= margin
// But without the subtraction to allow for INFINITY in comparison
bool WithinAbsMatcher::match(const double& matchee) const {
...
bool WithinUlpsMatcher::match(const double& matchee) const {
    switch (m_type) {
...
bool WithinRelMatcher::match(const double& matchee) const {
...
CasedString::CasedString(const std::string& str, CaseSensitive::Choice caseSensitivity) :
    m_caseSensitivity(caseSensitivity), m_str(adjustString(str)) {}
std::string CasedString::adjustString(const std::string& str) const {
...
StringMatcherBase::StringMatcherBase(const std::string& operation, const CasedString& comparator) :
...
EqualsMatcher::EqualsMatcher(const CasedString& comparator) :
    StringMatcherBase("equals", comparator) {}

bool EqualsMatcher::match(const std::string& source) const {
    return m_comparator.adjustString(source) == m_comparator.m_str;
}

ContainsMatcher::ContainsMatcher(const CasedString& comparator) :
    StringMatcherBase("contains", comparator) {}

bool ContainsMatcher::match(const std::string& source) const {
    return contains(m_comparator.adjustString(source), m_comparator.m_str);
}

StartsWithMatcher::StartsWithMatcher(const CasedString& comparator) :
    StringMatcherBase("starts with", comparator) {}

bool StartsWithMatcher::match(const std::string& source) const {
    return startsWith(m_comparator.adjustString(source), m_comparator.m_str);
}

EndsWithMatcher::EndsWithMatcher(const CasedString& comparator) :
    StringMatcherBase("ends with", comparator) {}

bool EndsWithMatcher::match(const std::string& source) const {
...
bool RegexMatcher::match(const std::string& matchee) const {
...
StdString::EqualsMatcher Equals(const std::string& str, CaseSensitive::Choice caseSensitivity) {
    return StdString::EqualsMatcher(StdString::CasedString(str, caseSensitivity));
}
StdString::ContainsMatcher Contains(const std::string& str, CaseSensitive::Choice caseSensitivity) {
    return StdString::ContainsMatcher(StdString::CasedString(str, caseSensitivity));
}
StdString::EndsWithMatcher EndsWith(const std::string& str, CaseSensitive::Choice caseSensitivity) {
    return StdString::EndsWithMatcher(StdString::CasedString(str, caseSensitivity));
}
StdString::StartsWithMatcher StartsWith(
    const std::string& str, CaseSensitive::Choice caseSensitivity) {
    return StdString::StartsWithMatcher(StdString::CasedString(str, caseSensitivity));
}

StdString::RegexMatcher Matches(const std::string& regex, CaseSensitive::Choice caseSensitivity) {
...
bool MessageInfo::operator==(const MessageInfo& other) const { return sequence == other.sequence; }
...
ScopedMessage::ScopedMessage(const MessageBuilder& builder) : m_info(builder.m_info), m_moved() {
...
void Capturer::captureValue(size_t index, const std::string& value) {
...
class RedirectedStreams {
  public:
    RedirectedStreams(const RedirectedStreams&) = delete;
    RedirectedStreams& operator=(const RedirectedStreams&) = delete;
...
bool operator==(const SimplePcg32& lhs, const SimplePcg32& rhs) {
...
std::vector<TestCase> sortTests(
    const IConfig& config, const std::vector<TestCase>& unsortedTestCases);

bool isThrowSafe(const TestCase& testCase, const IConfig& config);
bool matchTest(const TestCase& testCase, const TestSpec& testSpec, const IConfig& config);

void enforceNoDuplicateTestCases(const std::vector<TestCase>& functions);
...
class TestRegistry : public ITestCaseRegistry {
  public:
    ...
    const std::vector<TestCase>& getAllTests() const override;
    const std::vector<TestCase>& getAllTestsSorted(const IConfig& config) const override;
...
std::string extractClassName(const StringRef& classOrQualifiedMethodName);
...
class ReporterRegistry : public IReporterRegistry {
  public:
    ...
    IStreamingReporterPtr create(const std::string& name, const IConfigPtr& config) const override;

    void registerReporter(const std::string& name, const IReporterFactoryPtr& factory);
    void registerListener(const IReporterFactoryPtr& factory);

    const FactoryMap& getFactories() const override;
    const Listeners& getListeners() const override;
...
struct TagAlias {
    TagAlias(const std::string& _tag, SourceLineInfo _lineInfo);
...
class TagAliasRegistry : public ITagAliasRegistry {
  public:
    ...
    const TagAlias* find(const std::string& alias) const override;
    std::string expandAliases(const std::string& unexpandedTestSpec) const override;
    void add(const std::string& alias, const std::string& tag, const SourceLineInfo& lineInfo);
...
#if !defined(CATCH_CONFIG_DISABLE_EXCEPTIONS)
  public:
    void add(const std::exception_ptr& exception) noexcept;
...
template<
    typename SingletonImplT, typename InterfaceT = SingletonImplT,
...
class Singleton : SingletonImplT, public ISingleton {
  ...
  public:
    static auto get() -> const InterfaceT& { return *getInternal(); }
...
class RegistryHub : public IRegistryHub, public IMutableRegistryHub, private NonCopyable {
  public:  // IRegistryHub
    RegistryHub() = default;
    const IReporterRegistry& getReporterRegistry() const override { return m_reporterRegistry; }
    const ITestCaseRegistry& getTestCaseRegistry() const override { return m_testCaseRegistry; }
    const IExceptionTranslatorRegistry& getExceptionTranslatorRegistry() const override {
        return m_exceptionTranslatorRegistry;
    }
    const ITagAliasRegistry& getTagAliasRegistry() const override { return m_tagAliasRegistry; }
    const StartupExceptionRegistry& getStartupExceptionRegistry() const override {
        return m_exceptionRegistry;
    }

  public:  // IMutableRegistryHub
    void registerReporter(const std::string& name, const IReporterFactoryPtr& factory) override {
        m_reporterRegistry.registerReporter(name, factory);
    }
    void registerListener(const IReporterFactoryPtr& factory) override {
        m_reporterRegistry.registerListener(factory);
    }
    void registerTest(const TestCase& testInfo) override {
        m_testCaseRegistry.registerTest(testInfo);
    }
    void registerTranslator(const IExceptionTranslator* translator) override {
        m_exceptionTranslatorRegistry.registerTranslator(translator);
    }
    void registerTagAlias(
        const std::string& alias, const std::string& tag, const SourceLineInfo& lineInfo) override {
...
const IRegistryHub& getRegistryHub() { return RegistryHubSingleton::get(); }
...
IStreamingReporterPtr ReporterRegistry::create(
    const std::string& name, const IConfigPtr& config) const {
...
void ReporterRegistry::registerReporter(
    const std::string& name, const IReporterFactoryPtr& factory) {
    m_factories.emplace(name, factory);
}
void ReporterRegistry::registerListener(const IReporterFactoryPtr& factory) {
    m_listeners.push_back(factory);
}

const IReporterRegistry::FactoryMap& ReporterRegistry::getFactories() const { return m_factories; }
const IReporterRegistry::Listeners& ReporterRegistry::getListeners() const { return m_listeners; }
...
struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
    ...
    GeneratorTracker(
        const TestCaseTracking::NameAndLocation& nameAndLocation, TrackerContext& ctx,
        ITracker* parent) :
        TrackerBase(nameAndLocation, ctx, parent) {}
    ~GeneratorTracker();

    static GeneratorTracker& acquire(
        TrackerContext& ctx, const TestCaseTracking::NameAndLocation& nameAndLocation) {
        ...
        if (currentTracker.nameAndLocation() == nameAndLocation) {
            ...
            TestCaseTracking::ITrackerPtr childTracker =
    ...
    auto getGenerator() const -> const GeneratorBasePtr& override { return m_generator; }
    void setGenerator(GeneratorBasePtr&& generator) override { m_generator = std::move(generator); }
};
GeneratorTracker::~GeneratorTracker() {}
}  // namespace Generators

RunContext::RunContext(const IConfigPtr& _config, IStreamingReporterPtr&& reporter) :
...
void RunContext::testGroupStarting(
    const std::string& testSpec, std::size_t groupIndex, std::size_t groupsCount) {
...
Totals RunContext::runTest(const TestCase& testCase) {
    Totals prevTotals = m_totals;

    std::string redirectedCout;
    std::string redirectedCerr;

    const auto& testInfo = testCase.getTestCaseInfo();
...
void RunContext::assertionEnded(const AssertionResult& result) {
...
bool RunContext::sectionStarted(const SectionInfo& sectionInfo, Counts& assertions) {
...
auto RunContext::acquireGeneratorTracker(StringRef generatorName, const SourceLineInfo& lineInfo)
    -> IGeneratorTracker& {
...
void RunContext::sectionEnded(const SectionEndInfo& endInfo) {
...
void RunContext::sectionEndedEarly(const SectionEndInfo& endInfo) {
...
#if defined(CATCH_CONFIG_ENABLE_BENCHMARKING)
void RunContext::benchmarkPreparing(const std::string& name) {
    m_reporter->benchmarkPreparing(name);
}
void RunContext::benchmarkStarting(const BenchmarkInfo& info) {
    m_reporter->benchmarkStarting(info);
}
void RunContext::benchmarkEnded(const BenchmarkStats<>& stats) {
    m_reporter->benchmarkEnded(stats);
}
void RunContext::benchmarkFailed(const std::string& error) { m_reporter->benchmarkFailed(error); }
#endif  // CATCH_CONFIG_ENABLE_BENCHMARKING

void RunContext::pushScopedMessage(const MessageInfo& message) { m_messages.push_back(message); }

void RunContext::popScopedMessage(const MessageInfo& message) {
    m_messages.erase(std::remove(m_messages.begin(), m_messages.end(), message), m_messages.end());
}

void RunContext::emplaceUnscopedMessage(const MessageBuilder& builder) {
...
void RunContext::handleFatalErrorCondition(StringRef message) {
    ...
    const auto& testCaseInfo = m_activeTestCase->getTestCaseInfo();
    ...
    const auto& testInfo = m_activeTestCase->getTestCaseInfo();
...
void RunContext::handleExpr(
    const AssertionInfo& info, const ITransientExpression& expr, AssertionReaction& reaction) {
...
void RunContext::handleUnexpectedExceptionNotThrown(
    const AssertionInfo& info, AssertionReaction& reaction) {
    handleNonExpr(info, Catch::ResultWas::DidntThrowException, reaction);
}

void RunContext::handleUnexpectedInflightException(
    const AssertionInfo& info, const std::string& message, AssertionReaction& reaction) {
    m_lastAssertionInfo = info;
...
void RunContext::handleIncomplete(const AssertionInfo& info) {
    m_lastAssertionInfo = info;
...
void RunContext::handleNonExpr(
    const AssertionInfo& info, ResultWas::OfType resultType, AssertionReaction& reaction) {
    m_lastAssertionInfo = info;
...
Section::Section(const SectionInfo& info) :
...
SectionInfo::SectionInfo(const SourceLineInfo& _lineInfo, const std::string& _name) :
...
class Session : NonCopyable {
  public:
    ...
    int applyCommandLine(int argc, const char* const* argv);
#if defined(CATCH_CONFIG_WCHAR) && defined(_WIN32) && defined(UNICODE)
    int applyCommandLine(int argc, const wchar_t* const* argv);
#endif

    void useConfigData(const ConfigData& configData);

    template<typename CharT>
    int run(int argc, const CharT* const argv[]) {
    ...
    const clara::Parser& cli() const;
    void cli(const clara::Parser& newParser);
...
struct Version {
    ...
    Version(
        unsigned int _majorVersion, unsigned int _minorVersion, unsigned int _patchNumber,
        const char* const _branchName, unsigned int _buildNumber);
...
IStreamingReporterPtr createReporter(const std::string& reporterName, const IConfigPtr& config) {
...
IStreamingReporterPtr makeReporter(const std::shared_ptr<Config>& config) {
    ...
    const auto& listeners = Catch::getRegistryHub().getReporterRegistry().getListeners();
...
class TestGroup {
  public:
    explicit TestGroup(const std::shared_ptr<Config>& config) :
        m_config{config}, m_context{config, makeReporter(config)} {
        const auto& allTestCases = getAllTestCasesSorted(*m_config);
        m_matches = m_config->testSpec().matchesByFilter(allTestCases, *m_config);
        const auto& invalidArgs = m_config->testSpec().getInvalidArgs();

        if (m_matches.empty() && invalidArgs.empty()) {
            for (const auto& test : allTestCases)
    ...
    Totals execute() {
        ...
        if (!invalidArgs.empty()) {
            for (const auto& invalidArg : invalidArgs)
  ...
  private:
    using Tests = std::set<const TestCase*>;
...
void applyFilenamesAsTags(const Catch::IConfig& config) {
...
int Session::applyCommandLine(int argc, const char* const* argv) {
    if (m_startupExceptions) return 1;
...
#if defined(CATCH_CONFIG_WCHAR) && defined(_WIN32) && defined(UNICODE)
int Session::applyCommandLine(int argc, const wchar_t* const* argv) {
...
void Session::useConfigData(const ConfigData& configData) {
...
const clara::Parser& Session::cli() const { return m_cli; }
void Session::cli(const clara::Parser& newParser) { m_cli = newParser; }
...
int Session::runInternal() {
    ...
    CATCH_TRY {
        ...
        const auto totals = tests.execute();
        ...
        return (std::min)(
            MaxExitCode, (std::max)(totals.error, static_cast<int>(totals.assertions.failed)));
    }
#if !defined(CATCH_CONFIG_DISABLE_EXCEPTIONS)
    catch (std::exception& ex) {
...
#if !defined(CATCH_CONFIG_DISABLE_EXCEPTIONS)
namespace Catch {
void StartupExceptionRegistry::add(const std::exception_ptr& exception) noexcept {
...
const std::vector<std::exception_ptr>& StartupExceptionRegistry::getExceptions() const noexcept {
    return m_exceptions;
}
...
struct OutputDebugWriter {
    void operator()(const std::string& str) { writeToDebugConsole(str); }
};
...
bool startsWith(const std::string& s, char prefix) { return !s.empty() && s[0] == prefix; }
bool endsWith(const std::string& s, const std::string& suffix) {
    return s.size() >= suffix.size() && std::equal(suffix.rbegin(), suffix.rend(), s.rbegin());
}
bool endsWith(const std::string& s, char suffix) { return !s.empty() && s[s.size() - 1] == suffix; }
bool contains(const std::string& s, const std::string& infix) {
...
pluralise::pluralise(std::size_t count, const std::string& label) :
    m_count(count), m_label(label) {}

std::ostream& operator<<(std::ostream& os, const pluralise& pluraliser) {
...
namespace Catch {
StringRef::StringRef(const char* rawChars) noexcept :
    StringRef(rawChars, static_cast<StringRef::size_type>(std::strlen(rawChars))) {}

auto StringRef::c_str() const -> const char* {
    CATCH_ENFORCE(
        isNullTerminated(), "Called StringRef::c_str() on a non-null-terminated instance");
    return m_start;
}
auto StringRef::data() const noexcept -> const char* { return m_start; }
...
auto StringRef::operator==(const StringRef& other) const noexcept -> bool {
    return m_size == other.m_size && (std::memcmp(m_start, other.m_start, m_size) == 0);
}

auto operator<<(std::ostream& os, const StringRef& str) -> std::ostream& {
    return os.write(str.data(), str.size());
}

auto operator+=(std::string& lhs, const StringRef& rhs) -> std::string& {
...
namespace Catch {
TagAlias::TagAlias(const std::string& _tag, SourceLineInfo _lineInfo) :
...
RegistrarForTagAliases::RegistrarForTagAliases(
    const char* alias, const char* tag, const SourceLineInfo& lineInfo) {
...
const TagAlias* TagAliasRegistry::find(const std::string& alias) const {
...
std::string TagAliasRegistry::expandAliases(const std::string& unexpandedTestSpec) const {
    std::string expandedTestSpec = unexpandedTestSpec;
    for (const auto& registryKvp : m_registry) {
...
void TagAliasRegistry::add(
    const std::string& alias, const std::string& tag, const SourceLineInfo& lineInfo) {
    CATCH_ENFORCE(
        startsWith(alias, "[@") && endsWith(alias, ']'),
...
const ITagAliasRegistry& ITagAliasRegistry::get() { return getRegistryHub().getTagAliasRegistry(); }
...
namespace {
TestCaseInfo::SpecialProperties parseSpecialTag(const std::string& tag) {
...
bool isReservedTag(const std::string& tag) {
    return parseSpecialTag(tag) == TestCaseInfo::None && tag.size() > 0
           && !std::isalnum(static_cast<unsigned char>(tag[0]));
}
void enforceNotReservedTag(const std::string& tag, const SourceLineInfo& _lineInfo) {
    CATCH_ENFORCE(
        !isReservedTag(tag),
...
TestCase makeTestCase(
    ITestInvoker* _testCase, const std::string& _className, const NameAndTags& nameAndTags,
...
bool TestCase::operator<(const TestCase& other) const { return name < other.name; }

const TestCaseInfo& TestCase::getTestCaseInfo() const { return *this; }
...
struct TestHasher {
    ...
    uint32_t operator()(const TestCase& t) const {
...
std::vector<TestCase> sortTests(
    ...
    switch (config.runOrder()) {
        ...
        case RunTests::InRandomOrder: {
            ...
            using hashedTest = std::pair<TestHasher::hash_t, const TestCase*>;
            std::vector<hashedTest> indexed_tests;
            indexed_tests.reserve(unsortedTestCases.size());

            for (const auto& testCase : unsortedTestCases) {
            ...
            std::sort(
                indexed_tests.begin(),
                indexed_tests.end(),
                [](const hashedTest& lhs, const hashedTest& rhs) {
            ...
            for (const auto& hashed : indexed_tests) {
...
void TestRegistry::registerTest(const TestCase& testCase) {
...
const std::vector<TestCase>& TestRegistry::getAllTests() const { return m_functions; }
const std::vector<TestCase>& TestRegistry::getAllTestsSorted(const IConfig& config) const {
...
NameAndLocation::NameAndLocation(const std::string& _name, const SourceLineInfo& _location) :
...
TrackerBase::TrackerBase(
    const NameAndLocation& nameAndLocation, TrackerContext& ctx, ITracker* parent) :
...
void TrackerBase::addChild(const ITrackerPtr& child) { m_children.push_back(child); }

ITrackerPtr TrackerBase::findChild(const NameAndLocation& nameAndLocation) {
    auto it = std::find_if(
        m_children.begin(), m_children.end(), [&nameAndLocation](const ITrackerPtr& tracker) {
...
void TrackerBase::close() {
    ...
    switch (m_runState) {
        ...
        case ExecutingChildren:
            if (std::all_of(m_children.begin(), m_children.end(), [](const ITrackerPtr& t) {
                    return t->isComplete();
                }))
                m_runState = CompletedSuccessfully;
            break;
...
SectionTracker& SectionTracker::acquire(
    TrackerContext& ctx, const NameAndLocation& nameAndLocation) {
...
void SectionTracker::addInitialFilters(const std::vector<std::string>& filters) {
    if (!filters.empty()) {
...
void SectionTracker::addNextFilters(const std::vector<std::string>& filters) {
    if (filters.size() > 1) m_filters.insert(m_filters.end(), filters.begin() + 1, filters.end());
}

const std::vector<std::string>& SectionTracker::getFilters() const { return m_filters; }

const std::string& SectionTracker::trimmedName() const { return m_trimmed_name; }
...
NameAndTags::NameAndTags(const StringRef& name_, const StringRef& tags_) noexcept :
...
TestSpec::Pattern::Pattern(const std::string& name) : m_name(name) {}

TestSpec::Pattern::~Pattern() = default;

const std::string& TestSpec::Pattern::name() const { return m_name; }

TestSpec::NamePattern::NamePattern(const std::string& name, const std::string& filterString) :
    Pattern(filterString), m_wildcardPattern(toLower(name), CaseSensitive::No) {}

bool TestSpec::NamePattern::matches(const TestCaseInfo& testCase) const {
    return m_wildcardPattern.matches(testCase.name);
}

TestSpec::TagPattern::TagPattern(const std::string& tag, const std::string& filterString) :
    Pattern(filterString), m_tag(toLower(tag)) {}

bool TestSpec::TagPattern::matches(const TestCaseInfo& testCase) const {
    return std::find(begin(testCase.lcaseTags), end(testCase.lcaseTags), m_tag)
           != end(testCase.lcaseTags);
}

TestSpec::ExcludedPattern::ExcludedPattern(const PatternPtr& underlyingPattern) :
    Pattern(underlyingPattern->name()), m_underlyingPattern(underlyingPattern) {}

bool TestSpec::ExcludedPattern::matches(const TestCaseInfo& testCase) const {
    return !m_underlyingPattern->matches(testCase);
}

bool TestSpec::Filter::matches(const TestCaseInfo& testCase) const {
    return std::all_of(m_patterns.begin(), m_patterns.end(), [&](const PatternPtr& p) {
...
std::string TestSpec::Filter::name() const {
    std::string name;
    for (const auto& p : m_patterns) name += p->name();
    return name;
}

bool TestSpec::hasFilters() const { return !m_filters.empty(); }

bool TestSpec::matches(const TestCaseInfo& testCase) const {
    return std::any_of(
        m_filters.begin(), m_filters.end(), [&](const Filter& f) { return f.matches(testCase); });
}

TestSpec::Matches TestSpec::matchesByFilter(
    const std::vector<TestCase>& testCases, const IConfig& config) const {
    Matches matches(m_filters.size());
    std::transform(m_filters.begin(), m_filters.end(), matches.begin(), [&](const Filter& filter) {
...
TestSpecParser::TestSpecParser(const ITagAliasRegistry& tagAliases) : m_tagAliases(&tagAliases) {}

TestSpecParser& TestSpecParser::parse(const std::string& arg) {
    m_mode = None;
    m_exclusion = false;
...
std::string StringMaker<const char*>::convert(const char* str) {
    if (str) {
        return ::Catch::Detail::stringify(std::string{str});
    } else {
        return {"{null string}"};
...
std::string StringMaker<const wchar_t*>::convert(const wchar_t* str) {
    if (str) {
...
Totals Totals::operator-(const Totals& other) const {
    Totals diff;
...
Totals& Totals::operator+=(const Totals& other) {
    assertions += other.assertions;
    testCases += other.testCases;
    return *this;
}

Totals Totals::delta(const Totals& prevTotals) const {
...
WildcardPattern::WildcardPattern(
    const std::string& pattern, CaseSensitive::Choice caseSensitivity) :
...
bool WildcardPattern::matches(const std::string& str) const {
    switch (m_wildcard) {
        case NoWildcard:
...
std::string WildcardPattern::normaliseString(const std::string& str) const {
...
XmlWriter::ScopedElement& XmlWriter::ScopedElement::writeText(
    const std::string& text, XmlFormatting fmt) {
...
XmlWriter& XmlWriter::startElement(const std::string& name, XmlFormatting fmt) {
    ensureTagClosed();
    newlineIfNecessary();
    if (shouldIndent(fmt)) {
        m_os << m_indent;
...
XmlWriter::ScopedElement XmlWriter::scopedElement(const std::string& name, XmlFormatting fmt) {
...
XmlWriter& XmlWriter::writeText(const std::string& text, XmlFormatting fmt) {
    if (!text.empty()) {
...
XmlWriter& XmlWriter::writeComment(const std::string& text, XmlFormatting fmt) {
    ensureTagClosed();
    if (shouldIndent(fmt)) {
        m_os << m_indent;
    }
...
void XmlWriter::writeStylesheetRef(const std::string& url) {
...
TestEventListenerBase::TestEventListenerBase(const ReporterConfig& _config) :
...
void TestEventListenerBase::assertionStarting(const AssertionInfo&) {}

bool TestEventListenerBase::assertionEnded(const AssertionStats&) { return false; }
...
class AssertionPrinter {
  public:
    AssertionPrinter& operator=(const AssertionPrinter&) = delete;
    AssertionPrinter(const AssertionPrinter&) = delete;
    ...
    void print() {
        ...
        switch (result.getResultType()) {
            ...
            case ResultWas::ExpressionFailed:
                if (result.isOk())
                    printResultType(
                        Colour::ResultSuccess, failedString() + std::string(" - but was ok"));
                else
  ...
  private:
    ...
    void printResultType(Colour::Code colour, const std::string& passOrFail) const {
        if (!passOrFail.empty()) {
            {
                Colour colourGuard(colour);
                stream << ' ' << passOrFail;
    ...
    void printIssue(const std::string& issue) const { stream << ' ' << issue; }
...
void CompactReporter::noMatchingTestCases(const std::string& spec) {
    stream << "No test cases matched '" << spec << '\'' << std::endl;
}

void CompactReporter::assertionStarting(const AssertionInfo&) {}

bool CompactReporter::assertionEnded(const AssertionStats& _assertionStats) {
...
void CompactReporter::sectionEnded(const SectionStats& _sectionStats) {
...
void CompactReporter::testRunEnded(const TestRunStats& _testRunStats) {
...
class ConsoleAssertionPrinter {
  public:
    ConsoleAssertionPrinter& operator=(const ConsoleAssertionPrinter&) = delete;
    ConsoleAssertionPrinter(const ConsoleAssertionPrinter&) = delete;
    ConsoleAssertionPrinter(
        std::ostream& _stream, const AssertionStats& _stats, bool _printInfoMessages) :
        stream(_stream),
        stats(_stats),
        result(_stats.assertionResult),
        colour(Colour::None),
...
class Duration {
  ...
  public:
    ...
    friend auto operator<<(std::ostream& os, const Duration& duration) -> std::ostream& {
...
class TablePrinter {
  ...
  public:
    ...
    auto columnInfos() const -> const std::vector<ColumnInfo>& { return m_columnInfos; }
    ...
    template<typename T>
    friend TablePrinter& operator<<(TablePrinter& tp, const T& value) {
        tp.m_oss << value;
        return tp;
    }
...
ConsoleReporter::ConsoleReporter(const ReporterConfig& config) :
...
void ConsoleReporter::noMatchingTestCases(const std::string& spec) {
    stream << "No test cases matched '" << spec << '\'' << std::endl;
}

void ConsoleReporter::reportInvalidArguments(const std::string& arg) {
    stream << "Invalid Filter: " << arg << std::endl;
}

void ConsoleReporter::assertionStarting(const AssertionInfo&) {}

bool ConsoleReporter::assertionEnded(const AssertionStats& _assertionStats) {
    const AssertionResult& result = _assertionStats.assertionResult;
...
void ConsoleReporter::sectionStarting(const SectionInfo& _sectionInfo) {
    m_tablePrinter->close();
    m_headerPrinted = false;
    StreamingReporterBase::sectionStarting(_sectionInfo);
}
void ConsoleReporter::sectionEnded(const SectionStats& _sectionStats) {
    m_tablePrinter->close();
...
#if defined(CATCH_CONFIG_ENABLE_BENCHMARKING)
void ConsoleReporter::benchmarkPreparing(const std::string& name) {
...
void ConsoleReporter::benchmarkStarting(const BenchmarkInfo& info) {
    (*m_tablePrinter) << info.samples << ColumnBreak() << info.iterations << ColumnBreak();
    if (!m_config->benchmarkNoAnalysis())
        (*m_tablePrinter) << Duration(info.estimatedDuration) << ColumnBreak();
}
void ConsoleReporter::benchmarkEnded(const BenchmarkStats<>& stats) {
    if (m_config->benchmarkNoAnalysis()) {
...
void ConsoleReporter::benchmarkFailed(const std::string& error) {
    Colour colour(Colour::Red);
    (*m_tablePrinter) << "Benchmark failed (" << error << ')' << ColumnBreak() << RowBreak();
}
#endif  // CATCH_CONFIG_ENABLE_BENCHMARKING

void ConsoleReporter::testCaseEnded(const TestCaseStats& _testCaseStats) {
    m_tablePrinter->close();
    StreamingReporterBase::testCaseEnded(_testCaseStats);
    m_headerPrinted = false;
}
void ConsoleReporter::testGroupEnded(const TestGroupStats& _testGroupStats) {
...
void ConsoleReporter::testRunEnded(const TestRunStats& _testRunStats) {
    printTotalsDivider(_testRunStats.totals);
    printTotals(_testRunStats.totals);
    stream << std::endl;
    StreamingReporterBase::testRunEnded(_testRunStats);
}
void ConsoleReporter::testRunStarting(const TestRunInfo& _testInfo) {
...
void ConsoleReporter::printClosedHeader(const std::string& _name) {
    printOpenHeader(_name);
    stream << getLineOfChars<'.'>() << '\n';
}
void ConsoleReporter::printOpenHeader(const std::string& _name) {
    stream << getLineOfChars<'-'>() << '\n';
    {
        Colour colourGuard(Colour::Headers);
...
// if string has a : in first line will set indent to follow it on
// subsequent lines
void ConsoleReporter::printHeaderString(const std::string& _string, std::size_t indent) {
...
void ConsoleReporter::printTotals(const Totals& totals) {
    if (totals.testCases.total() == 0) {
...
void ConsoleReporter::printSummaryRow(
    const std::string& label, const std::vector<SummaryColumn>& cols, std::size_t row) {
    for (auto col : cols) {
...
void ConsoleReporter::printTotalsDivider(const Totals& totals) {
    if (totals.testCases.total() > 0) {
...
std::string fileNameTag(const std::vector<std::string>& tags) {
    auto it = std::find_if(
        begin(tags), end(tags), [](const std::string& tag) { return tag.front() == '#'; });
...
JunitReporter::JunitReporter(const ReporterConfig& _config) :
...
void JunitReporter::noMatchingTestCases(const std::string& /*spec*/) {}

void JunitReporter::testRunStarting(const TestRunInfo& runInfo) {
    CumulativeReporterBase::testRunStarting(runInfo);
    xml.startElement("testsuites");
}

void JunitReporter::testGroupStarting(const GroupInfo& groupInfo) {
    suiteTimer.start();
...
void JunitReporter::testCaseStarting(const TestCaseInfo& testCaseInfo) {
    m_okToFail = testCaseInfo.okToFail();
}

bool JunitReporter::assertionEnded(const AssertionStats& assertionStats) {
    if (assertionStats.assertionResult.getResultType() == ResultWas::ThrewException && !m_okToFail)
        unexpectedExceptions++;
    return CumulativeReporterBase::assertionEnded(assertionStats);
}

void JunitReporter::testCaseEnded(const TestCaseStats& testCaseStats) {
    stdOutForSuite += testCaseStats.stdOut;
    stdErrForSuite += testCaseStats.stdErr;
    CumulativeReporterBase::testCaseEnded(testCaseStats);
}

void JunitReporter::testGroupEnded(const TestGroupStats& testGroupStats) {
...
void JunitReporter::writeGroup(const TestGroupNode& groupNode, double suiteTime) {
    ...
    // Write test cases
    for (const auto& child : groupNode.children) writeTestCase(*child);

    xml.scopedElement("system-out").writeText(trim(stdOutForSuite), XmlFormatting::Newline);
    xml.scopedElement("system-err").writeText(trim(stdErrForSuite), XmlFormatting::Newline);
}

void JunitReporter::writeTestCase(const TestCaseNode& testCaseNode) {
...
void JunitReporter::writeAssertions(const SectionNode& sectionNode) {
    for (const auto& assertion : sectionNode.assertions) writeAssertion(assertion);
}

void JunitReporter::writeAssertion(const AssertionStats& stats) {
...
void ListeningReporter::noMatchingTestCases(const std::string& spec) {
...
void ListeningReporter::reportInvalidArguments(const std::string& arg) {
    for (const auto& listener : m_listeners) {
...
#if defined(CATCH_CONFIG_ENABLE_BENCHMARKING)
void ListeningReporter::benchmarkPreparing(const std::string& name) {
    for (const auto& listener : m_listeners) {
        listener->benchmarkPreparing(name);
    }
    m_reporter->benchmarkPreparing(name);
}
void ListeningReporter::benchmarkStarting(const BenchmarkInfo& benchmarkInfo) {
    for (const auto& listener : m_listeners) {
        listener->benchmarkStarting(benchmarkInfo);
    }
    m_reporter->benchmarkStarting(benchmarkInfo);
}
void ListeningReporter::benchmarkEnded(const BenchmarkStats<>& benchmarkStats) {
    for (const auto& listener : m_listeners) {
...
void ListeningReporter::benchmarkFailed(const std::string& error) {
    for (const auto& listener : m_listeners) {
...
void ListeningReporter::testRunStarting(const TestRunInfo& testRunInfo) {
    for (const auto& listener : m_listeners) {
...
void ListeningReporter::testGroupStarting(const GroupInfo& groupInfo) {
    for (const auto& listener : m_listeners) {
...
void ListeningReporter::testCaseStarting(const TestCaseInfo& testInfo) {
    for (const auto& listener : m_listeners) {
...
void ListeningReporter::sectionStarting(const SectionInfo& sectionInfo) {
    for (const auto& listener : m_listeners) {
...
void ListeningReporter::assertionStarting(const AssertionInfo& assertionInfo) {
    for (const auto& listener : m_listeners) {
...
// The return value indicates if the messages buffer should be cleared:
bool ListeningReporter::assertionEnded(const AssertionStats& assertionStats) {
    for (const auto& listener : m_listeners) {
...
void ListeningReporter::sectionEnded(const SectionStats& sectionStats) {
    for (const auto& listener : m_listeners) {
...
void ListeningReporter::testCaseEnded(const TestCaseStats& testCaseStats) {
    for (const auto& listener : m_listeners) {
...
void ListeningReporter::testGroupEnded(const TestGroupStats& testGroupStats) {
    for (const auto& listener : m_listeners) {
...
void ListeningReporter::testRunEnded(const TestRunStats& testRunStats) {
    for (const auto& listener : m_listeners) {
...
void ListeningReporter::skipTest(const TestCaseInfo& testInfo) {
    for (const auto& listener : m_listeners) {
...
namespace Catch {
XmlReporter::XmlReporter(const ReporterConfig& _config) :
...
void XmlReporter::writeSourceInfo(const SourceLineInfo& sourceInfo) {
    m_xml.writeAttribute("filename", sourceInfo.file).writeAttribute("line", sourceInfo.line);
}

void XmlReporter::noMatchingTestCases(const std::string& s) {
    StreamingReporterBase::noMatchingTestCases(s);
}

void XmlReporter::testRunStarting(const TestRunInfo& testInfo) {
...
void XmlReporter::testGroupStarting(const GroupInfo& groupInfo) {
    StreamingReporterBase::testGroupStarting(groupInfo);
    m_xml.startElement("Group").writeAttribute("name", groupInfo.name);
}

void XmlReporter::testCaseStarting(const TestCaseInfo& testInfo) {
...
void XmlReporter::sectionStarting(const SectionInfo& sectionInfo) {
...
void XmlReporter::assertionStarting(const AssertionInfo&) {}

bool XmlReporter::assertionEnded(const AssertionStats& assertionStats) {
    ...
    if (includeResults || result.getResultType() == ResultWas::Warning) {
        ...
        for (const auto& msg : assertionStats.infoMessages) {
...
void XmlReporter::sectionEnded(const SectionStats& sectionStats) {
...
void XmlReporter::testCaseEnded(const TestCaseStats& testCaseStats) {
...
void XmlReporter::testGroupEnded(const TestGroupStats& testGroupStats) {
...
void XmlReporter::testRunEnded(const TestRunStats& testRunStats) {
...
#if defined(CATCH_CONFIG_ENABLE_BENCHMARKING)
void XmlReporter::benchmarkPreparing(const std::string& name) {
    m_xml.startElement("BenchmarkResults").writeAttribute("name", name);
}

void XmlReporter::benchmarkStarting(const BenchmarkInfo& info) {
...
void XmlReporter::benchmarkEnded(const BenchmarkStats<>& benchmarkStats) {
...
void XmlReporter::benchmarkFailed(const std::string& error) {
